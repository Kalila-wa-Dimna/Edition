...
export class DownloadsService {
  ...
  unitTableEndpoint = (collationName: string) => `${this.config.dataEndPoint}collations/${collationName}`
  ...
  async downloadUnitTable() {
    ...
    try {
      const data = await firstValueFrom(this.httpClient.get(endpoint+'/unit_table.csv', { responseType: 'text' }));
...
async downloadEditionTable() {
  ...
  const endpoint = this.unitTableEndpoint(this.collationName);
  try {
    const data = await firstValueFrom(this.httpClient.get(endpoint+'/segment_tokens.csv', { responseType: 'text' }));
    const blob = new Blob([data], { type: 'text/csv' });
    const url = window.URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `${this.collationName}_edition.csv`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    window.URL.revokeObjectURL(url);
